...
export const buildCampaignText = (partner) => {
  ...
  const { createUrl } = buildPartnerLinks(partner);
  const ps = partner?.bookingUrl
    ? `P.S. Want to lock a table now? ${partner.bookingUrl}`
    : "P.S. Want to lock a table now? Give us a call and we'll help.";

  return [
    `Celebrate at ${venue}${city ? ` in ${city}` : ''}. Pick a few dates, share a link, and get your group to vote Best/Maybe/No. No logins.`,
    `Start here: ${createUrl}`,
    ps,
  ].join('\n');
};
...
export const normalizePartnerRecord = (partner, slug) => {
  ...
  const createdAt = partner.createdAt?.toDate ? partner.createdAt.toDate().toISOString() : partner.createdAt || null;
  return { ...partner, slug, createdAt };
};
