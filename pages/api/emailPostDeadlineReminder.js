// /pages/api/emailPostDeadlineReminder.js

export default async function handler(req, res) {
  const { organiserEmail, organiserName, eventTitle, location, pollId } = req.body;
  ...
  const resultsLink = `${baseUrl}/results/${pollId}`;
  const editLink = `${baseUrl}/edit/${pollId}`;
  ...
  const html = `
    ...
    <p style="font-size: 16px;">
      The voting deadline has passed for your event <strong>"${eventTitle}"</strong> in <strong>${location}</strong>.
    </p>

    <p style="font-size: 16px;">
      Ready to lock in the best date — or extend the deadline to get more votes?
    </p>
