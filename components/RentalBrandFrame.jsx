...
export default function RentalBrandFrame({ property, children, showLogoAtTop = true }) {
  const {
    propertyName = 'Set The Date Property',
    locationText,
    slug,
    accentColor = FALLBACK_BRAND,
    logoUrl,
    heroImageUrl,
    images,
    metaDescription,
  ...
  const accent = accentColor || FALLBACK_BRAND;
  const title = `${propertyName} x Set The Date`;
  const description =
    metaDescription || `Plan your next stay at ${propertyName}${locationText ? ` in ${locationText}` : ''} with Set The Date.`;
  const shareUrl = slug ? `${SHARE_BASE_URL}/rentals/p/${slug}` : SHARE_BASE_URL;
  const ogImage = getPartnerOgImage(
    {
      venuePhotoGallery: Array.isArray(images) ? images : [],
      venuePhotoUrl: heroImageUrl,
      logoUrl,
    },
    FALLBACK_OG_IMAGE
  );
          ...
          >
            {logoUrl && showLogoAtTop && (
              <div className="flex justify-center">
                <a
                  href={propertyHref}
                  aria-label={`${propertyName} homepage`}
